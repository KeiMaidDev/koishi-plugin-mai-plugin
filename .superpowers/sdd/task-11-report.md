...
| Music discovery | random, name/alias, constant/range, fitted, designer, version, artist, regex, BPM, combo | All listed triggers; range and pagination paths | Empty, malformed, unsafe, overlength, timeout, threshold-plus-one, and no-result paths; regex executes in a terminated worker | Complete |
| Alias management | add alias, administrator delete | Vote/add and authority-4 delete; `mai.alias-remove` declares `authority:4` | Missing args and non-admin delete | Complete |
...
| Lifecycle / callbacks | commands, middleware, interaction listener, callback state disposal | `interaction/button` dispatches scoped Task 10 callbacks and sends rendered handler output | User/channel/authority/permission mismatches are rejected; disposal unregisters commands, middleware, listener, and callback state | Complete |
...
- Review RED/GREEN cycles cover ReDoS timeout, real button ingress, 50-record score-list pagination, pending replay, parser-validated compatibility, mention/privacy mapping, historical ratings, random courses, search threshold, alias authority, and daily determinism.
- Focused GREEN: `npx vitest run tests/integration/commands-core.spec.ts` passed 180/180 cases.

## Verification

| Check | Result |
| --- | --- |
| Focused command integration | Passed, 180/180 |
| Affected suites | Passed, 213/213 across aliases, combo query, QQ message, QueryService, rating, render templates, and plugin lifecycle |
| Production build | Passed: TypeScript declarations plus Vite production bundle; final bundle 304.04 kB |
| Full test suite | Passed, 569/569 across 15 files |
...
- User regular expressions are length-limited, reject malformed/known high-risk structures, and execute in a worker thread with a strict timeout and termination. The worker boundary bounds event-loop impact; it is not a proof that every accepted expression has linear complexity.
