...
封面、头像、姓名框和试听音频使用 LXNS 游戏资源站。素材仅在实际使用时下载，并持久缓存到 `resourceSync.cacheDir/assets`；歌曲查询会先读取本地封面缓存，再通过 Koishi Assets 映射为 QQ Raw Markdown 可访问的图片直链，Assets 不可用或映射失败时回退为普通图片消息。已有缓存会直接复用，远程资源暂时不可用时不会覆盖本地文件。插件不会在启动时批量镜像全部素材，以避免触发官方访问频率限制。
