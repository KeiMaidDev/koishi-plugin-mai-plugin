...
远端歌曲别名只从 LXNS `GET /api/v0/maimai/alias/list` 获取，水鱼不作为歌曲别名数据源。有效缓存位于 `resourceSync.cacheDir/aliases.json`；存在有效缓存时不会访问网络，当前也不会自动刷新。远端响应和本地缓存均受 8 MiB 文件大小、20,000 个源别名总数、20,000 条记录、每首曲目 128 个别名及单个别名 128 个 Unicode 码点的安全上限约束，任一超限会拒绝整包数据。缓存缺失、损坏、不包含有效别名或超过安全上限时，插件会重新拉取并原子保存；拉取或保存失败不会阻塞启动，也不会覆盖原缓存。搜索时依次匹配正式标题、远端别名和用户已批准别名。远端别名缓存流程不会写入 `mai_alias` 或 `mai_alias_vote`。
